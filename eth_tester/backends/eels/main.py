...
class EELSBackend(BaseChainBackend):
    ...
    def get_nonce(
        ...
        with self._state_context_manager(block_number):
            return int(self.fork.get_account(self.chain.state, address).nonce)

    def get_balance(
        self, address: RequestHexStr, block_number: RequestBlockIdentifier
    ) -> int:
        with self._state_context_manager(block_number):
            return int(self.fork.get_account(self.chain.state, address).balance)

    def get_code(
        self, address: RequestHexStr, block_number: RequestBlockIdentifier
    ) -> bytes:
        with self._state_context_manager(block_number):
            return bytes(self.fork.get_account(self.chain.state, address).code)
